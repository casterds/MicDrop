...
import stacksGalleryLogo from "../images/MicDrop.png";
...
const GalleryStacks = () => {
    ...
    const handlerSearchByAddress = async (e: any, inputAddress: any) => {
        ...
        if(!inputAddress.trim()){
            setInputAddress("Please insert an Ethereum address or an ENS name");
        }  else if (inputAddress.trim().toUpperCase().substring(0, 2) !== "0X" && !inputAddress.includes(".")){
            setInputAddressError("Invalid Ethereum address or ENS name.");
        }  else {
            setLoading(true);

            if(inputAddress.trim().toUpperCase().substring(0, 2) === "0X"){
                setLoadingText("Check Ethereum address.");
                // Checking NFTs by using address.
                let result  = await checkEthAddress(inputAddress.trim(), setAddress, setLoading);
                ...
                // Checking NFTs by using ENS name
                console.log("ENS")
                if(inputAddress.trim().include(".eth")){
                    setLoading(true);
                    setLoadingText("Check ENS name.");
                    let result  = await checkENSName(inputAddress.trim(), setAddress, setDomain, setLoading);
                    if(result?.error){
                        setInputAddress(result?.error);
                        setLoading(false)
                    }
                }
                                ...
                                <div style={{display: 'flex', justifyContent: 'space-around', alignItems: 'center'}}>
                                    <input onChange={(e: any) => {
                                        ...
                                        setInputAddress(e.target.value)
                                           ...
                                           type="text"
                                           id="fname"
                                           placeholder='ETH address or a ENS name'/>
